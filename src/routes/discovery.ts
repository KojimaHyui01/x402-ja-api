...
export function buildInstructions(cfg: Config): string {
  const lines = [
    ...
    `Try before you pay: send the header \`X-Free-Tier: 1\` and each client IP gets ${cfg.FREE_QUOTA_PER_DAY} free calls per UTC day; after that (or without the header) a 402 challenge is returned.`,
    "",
    "## How to call",
    "1. Send the request with `X-Free-Tier: 1`. If you are within the free quota you get 200 immediately.",
